...
const SettingsPage: React.FC<AppProps> = ({ appName }) => {
  ...
  return (
    <>
      <SEOHead title={`${t('settings.title')} - ${appName}`} description="Appearance settings" />

      <div className="max-w-3xl mx-auto px-4 py-12">
        <h1 className={`${textVariants.heading.display.md()} mb-2`}>{t('settings.title')}</h1>
        ...
        <div className="space-y-4">
          ...
          <SettingRow title={t('settings.theme')}>
            <Select value={theme} onValueChange={value => setTheme(value as Theme)}>
              <SelectTrigger aria-label={t('settings.theme')} className="min-w-[10rem]">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value={Theme.LIGHT}>{t('settings.light')}</SelectItem>
                <SelectItem value={Theme.DARK}>{t('settings.dark')}</SelectItem>
                <SelectItem value={Theme.SYSTEM}>{t('settings.system')}</SelectItem>
              </SelectContent>
            </Select>
          </SettingRow>
          ...
          <SettingRow title={t('settings.fontSize')}>
            <Select value={fontSize} onValueChange={value => setFontSize(value as FontSize)}>
              <SelectTrigger aria-label={t('settings.fontSize')} className="min-w-[10rem]">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value={FontSize.SMALL}>{t('settings.small')}</SelectItem>
                <SelectItem value={FontSize.MEDIUM}>{t('settings.medium')}</SelectItem>
                <SelectItem value={FontSize.LARGE}>{t('settings.large')}</SelectItem>
              </SelectContent>
            </Select>
          </SettingRow>
